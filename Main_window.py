...
def create_main_menu():
    ...
    main_menu.add_command(label="Электричество", command=manage_databases)
    main_menu.add_command(label="Тепло", command=manage_tables)
    main_menu.add_command(label="Вода", command=manage_columns)
    main_menu.add_command(label="Комментарии", command=manage_content)
    main_menu.add_separator()
    main_menu.add_command(label="Выход", command=window.destroy)
    
    menu_bar.add_cascade(label="Показания", menu=main_menu)
...
#     button_6D_1 = ttk.Button(contentframe_6D, text='Show Content', command=show_table_content_cm)
#     button_6D_1.grid(row=0, column=0, sticky='nsew', padx=5, pady=5)
#     label_6D_1 = ttk.Label(contentframe_6D, text='Press to show table content', background='#7BCCB5')
...
# # Function to add the chosen file to the database
# # def add_file_cm():
# #     file_name = chosen_file_name_cm.get()
...
# #     if file_name == "Select Column for a file name" or file_content == "Select Column for a file content":
# #         messagebox.showerror("Error", "Please choose a file and content columns.")
# #         return

# #     try:
# #         # Add your logic for adding the file to the database
# #         # For example, use the selected_database, selected_table, file_name, and file_content
